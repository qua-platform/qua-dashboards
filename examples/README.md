...
In this directory, we integrate video mode with various simulations. In [example_video_mode_random.py](video_mode.simulated_examples.example_video_mode_random.py), a RandomSimulator is instantiated to collect random data to display in Video Mode. One also has the options to instantiate a simulator, as in the example shown in [example_video_mode_simulation.py](video_mode.simulated_examples.example_video_mode_simulation.py), which integrates a Qarray simulation. To implement your own simulation, simply subclass the BaseSimulator in [src/qua_dashboards/video_mode/inner_loop_actions/simulators/(base_simulator.py](qua_dashboards/video_mode/inner_loop_actions/simulators/base_simulator.py), ensuring that the measure_data function returns the data in the correct format. 
