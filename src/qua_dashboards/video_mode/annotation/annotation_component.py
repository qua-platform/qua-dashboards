...
class AnnotationComponent(BaseComponent):
    ...
    _RETURN_BUTTON_ID = "return-button"
        ...
        return dbc.Card(
            dbc.CardBody(
                [
                    ...
                    dbc.Button(
                        "Return to Live View",
                        id=self._get_id(self._RETURN_BUTTON_ID),
                        color="primary",
                        size="sm",
    ...
    def register_callbacks(self, app: Dash, orchestrator_stores: Dict[str, str]):
        ...
        Args:
            ...
            orchestrator_stores: A dictionary mapping store purposes (e.g.,
                                'snapshot_store', 'signal_store') to their
                                actual IDs generated by the orchestrator.
                                Expected keys: 'snapshot_store', 'signal_store'.
        """
        # Validate required store IDs
        if (
            "snapshot_store" not in orchestrator_stores
            or "signal_store" not in orchestrator_stores
        ):
            raise ValueError(
                "Orchestrator store IDs for 'snapshot_store' and 'signal_store' are required."
            )
        ...
        self._register_return_callback(app, orchestrator_stores["signal_store"])
    ...
    def _register_return_callback(self, app: Dash, signal_store_id: str):
        """Callback to signal the orchestrator to return to live view."""

        @app.callback(
            Output(signal_store_id, "data"),  # Output to orchestrator's signal store
            Input(self._get_id(self._RETURN_BUTTON_ID), "n_clicks"),
            State(signal_store_id, "data"),  # Read current signal value
            prevent_initial_call=True,
        )
        def signal_return_to_live(n_clicks, current_signal):
            logger.debug(f"{self.component_id}: Signaling return to live view.")
            # Increment signal or use a timestamp/random value to ensure change detection
