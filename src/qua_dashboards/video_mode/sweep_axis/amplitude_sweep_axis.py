...
class AmplitudeSweepAxis(BaseSweepAxis):
    """Class representing an Amplitude sweep axis.
    Takes a pulse object. apply() function returns the necessary amplitude scaling to achieve an absolute pulse amplitude.
    ...
    Attributes:
        ...
        offset_parameter: Parameter which provides the offset; in this case, should be the pulse object.
    """
    ...
    def create_axis_layout(self, min_span: float, max_span: Optional[float] = None):
        ...
        ids = {
            ...
            "dbm-toggle": {
                "type": "toggle",
                "index": f"{self.component_id}::dbm-toggle",
            },
        }
        toggle = dbc.Row(
            [
        ...
        offset_input = create_input_field(
            id=ids["offset"],
            label="Offset",
            value=self.offset,
        ...
        body.children.extend([offset_input, toggle])
    ...
    def update_parameters(self, parameters: Dict[str, Dict[str, Any]]) -> ModifiedFlags:
        ...
        if "offset" in params and self.offset != params["offset"]:
            ...
            flags |= (
                ...
                | ModifiedFlags.CONFIG_MODIFIED
            )

        if "dbm-toggle" in params:
            raw = params["dbm-toggle"]
            new_dbm = (
                bool(raw) if isinstance(raw, (list, tuple, set)) else (raw == "on")
            )
